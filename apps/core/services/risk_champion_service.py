"""
Service for Risk Champion Appointment workflow integration (FIMS pattern).
...
Template: grc.risk_champion_appointment (3-stage: rmo_draft → rmqam_review → dg_signature)
"""
import logging
from django.db import transaction
from apps.core.models import RiskChampionAppointment
from apps.infrastructure.external.orchestration_client import OrchestrationClient

logger = logging.getLogger(__name__)


class RiskChampionAppointmentService:

    WORKFLOW_TEMPLATE_CODE = "grc.risk_champion_appointment"
    ...
    def submit_for_approval(self, appointment_id: str, submitter_id: str):
        """
        Submit a Risk Champion Appointment to Work Orchestration Service.
        """
        entity = RiskChampionAppointment.objects.select_for_update().get(id=appointment_id)

        if entity.workflow_plan_id:
            logger.info(
                "RiskChampionAppointment %s already has workflow plan %s — skipping",
        ...
        if result and result.plan_id:
            ...
            logger.info(
                "Started workflow plan %s for RiskChampionAppointment %s (stage: %s)",
                result.plan_id, entity.id, result.current_stage_name,
            )
        else:
            logger.error("Failed to start workflow for RiskChampionAppointment %s", entity.id)

        return entity

    def get_workflow_status(self, appointment_id: str):
        """Get current workflow status."""
        entity = RiskChampionAppointment.objects.get(id=appointment_id)
    ...
    def advance_workflow_stage(self, appointment_id: str, action: str, actor_id: str, comment: str = '') -> dict:
        ...
        if not entity.workflow_plan_id:
            raise ValueError("No active workflow for this RiskChampionAppointment")
    ...
    @transaction.atomic
    def cancel_workflow_plan(self, appointment_id: str, actor_id: str, reason: str = '') -> RiskChampionAppointment:
