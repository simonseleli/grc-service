"""
Service for Risk Treatment Action Plan (RTAP) workflow integration (FIMS pattern).

Template: grc.rtap_approval
  (4-stage: rmqam_review → management_discussion → committee_review → commission_approval)
"""
import logging
from django.db import transaction
from apps.core.models import RiskTreatmentActionPlan
...
class RTAPService:
    ...
    WORKFLOW_TEMPLATE_CODE = "grc.rtap_approval"

    def __init__(self):
        self.workflow_client = OrchestrationClient()

    @transaction.atomic
    def submit_for_approval(self, rtap_id: str, submitter_id: str):
        """Submit a Risk Treatment Action Plan to Work Orchestration Service."""
        entity = RiskTreatmentActionPlan.objects.select_for_update().get(id=rtap_id)

        if entity.workflow_plan_id:
            logger.info(
                "RiskTreatmentActionPlan %s already has workflow plan %s — skipping",
        ...
        if result and result.plan_id:
            ...
            logger.info(
                "Started workflow plan %s for RiskTreatmentActionPlan %s (stage: %s)",
                result.plan_id, entity.id, result.current_stage_name,
            )
        else:
            logger.error("Failed to start workflow for RiskTreatmentActionPlan %s", entity.id)

        return entity

    def get_workflow_status(self, rtap_id: str):
        """Get current workflow status."""
        entity = RiskTreatmentActionPlan.objects.get(id=rtap_id)
    ...
    def get_workflow_history(self, rtap_id: str):
    ...
    @transaction.atomic
    def advance_workflow_stage(self, rtap_id: str, action: str, actor_id: str, comment: str = '') -> dict:
        """Execute a workflow action."""
        entity = RiskTreatmentActionPlan.objects.select_for_update().get(id=rtap_id)

        if not entity.workflow_plan_id:
            raise ValueError("No active workflow for this RiskTreatmentActionPlan")
    ...
    @transaction.atomic
    def cancel_workflow_plan(self, rtap_id: str, actor_id: str, reason: str = '') -> RiskTreatmentActionPlan:
