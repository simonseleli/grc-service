"""
Service for FilingDefendant / FilingPlaintiff workflow integration (FIMS pattern).
Template: grc.legal_filing_approval
  Stages: filing_officer_review → legal_manager_review → dg_filing_review → filing_confirmed
Handles both defendant and plaintiff sides via `side` parameter.
"""
import logging
from django.db import transaction
from apps.core.models import FilingDefendant, FilingPlaintiff
from apps.infrastructure.external.orchestration_client import OrchestrationClient
from shared.constants.event_types import LEGAL_FILING_EVENTS

logger = logging.getLogger(__name__)


def _publish_filing_event(event_type, entity, side, actor_id):
    """Best-effort Kafka domain event for filing lifecycle."""
    try:
        ...
        messaging_service.publish_legal_filing_event(
            event_type=event_type,
            filing_id=entity.id,
            additional_data={
                'case_id': str(getattr(entity, case_field, '') or ''),
                'filing_type': getattr(entity, 'filing_type', ''),
                'reference_number': getattr(entity, 'reference_number', ''),
                'approved_by': actor_id,
                'user_id': actor_id,
            },
        )
    except Exception as exc:
        logger.warning("Failed to publish %s event for filing %s: %s", event_type, entity.id, exc)

MODEL_MAP = {
    'defendant': FilingDefendant,
    'plaintiff': FilingPlaintiff,
}

# GAP-06: Map completed stage name → entity status
FILING_STAGE_STATUS_MAP = {
...
class LegalFilingService:
    ...
    def advance_workflow_stage(
        ...
        # GAP-06: Map completed stage → entity status using FILING_STAGE_STATUS_MAP
        completed_stage_name = entity.workflow_stage  # stage that was just acted on
        new_entity_status = FILING_STAGE_STATUS_MAP.get(completed_stage_name)
        if new_entity_status and result.new_status == 'completed':
        ...
        if result.plan_status == 'completed':
            _publish_filing_event(LEGAL_FILING_EVENTS['FILING_APPROVED'], entity, side, actor_id)
