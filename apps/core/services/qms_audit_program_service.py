"""
Service for QMS Audit Program workflow integration (FIMS pattern).

Template: grc.qms_audit_program_approval (2-stage: rmo_submit → rmqam_approve)
"""
import logging
from django.db import transaction
from apps.core.models import QMSAuditProgram
from apps.infrastructure.external.orchestration_client import OrchestrationClient

logger = logging.getLogger(__name__)


class QMSAuditProgramService:

    WORKFLOW_TEMPLATE_CODE = "grc.qms_audit_program_approval"

    def __init__(self):
        self.workflow_client = OrchestrationClient()

    @transaction.atomic
    def submit_for_approval(self, program_id: str, submitter_id: str):
        """Submit a QMS Audit Program to Work Orchestration Service."""
        entity = QMSAuditProgram.objects.select_for_update().get(id=program_id)

        if entity.workflow_plan_id:
            logger.info(
                "QMSAuditProgram %s already has workflow plan %s — skipping",
        ...
        if result and result.plan_id:
            ...
            logger.info(
                "Started workflow plan %s for QMSAuditProgram %s (stage: %s)",
                result.plan_id, entity.id, result.current_stage_name,
            )
        else:
            logger.error("Failed to start workflow for QMSAuditProgram %s", entity.id)

        return entity

    def get_workflow_status(self, program_id: str):
        """Get current workflow status."""
        entity = QMSAuditProgram.objects.get(id=program_id)
    ...
    def advance_workflow_stage(self, program_id: str, action: str, actor_id: str, comment: str = '') -> dict:
        ...
        if not entity.workflow_plan_id:
            raise ValueError("No active workflow for this QMSAuditProgram")
    ...
    @transaction.atomic
    def cancel_workflow_plan(self, program_id: str, actor_id: str, reason: str = '') -> QMSAuditProgram:
