"""
Service for QMS Audit Plan workflow integration (FIMS pattern).

Template: grc.qms_audit_plan_approval (2-stage: rmo_submit → rmqam_approve)
"""
import logging
from django.db import transaction
from apps.core.models import QMSAuditPlan
from apps.infrastructure.external.orchestration_client import OrchestrationClient

logger = logging.getLogger(__name__)


class QMSAuditPlanService:

    WORKFLOW_TEMPLATE_CODE = "grc.qms_audit_plan_approval"

    def __init__(self):
        self.workflow_client = OrchestrationClient()

    @transaction.atomic
    def submit_for_approval(self, plan_id: str, submitter_id: str):
        """Submit a QMS Audit Plan to Work Orchestration Service."""
        entity = QMSAuditPlan.objects.select_for_update().get(id=plan_id)

        if entity.workflow_plan_id:
            logger.info(
                "QMSAuditPlan %s already has workflow plan %s — skipping",
        ...
        if result and result.plan_id:
            ...
            logger.info(
                "Started workflow plan %s for QMSAuditPlan %s (stage: %s)",
                result.plan_id, entity.id, result.current_stage_name,
            )
        else:
            logger.error("Failed to start workflow for QMSAuditPlan %s", entity.id)

        return entity

    def get_workflow_status(self, plan_id: str):
        """Get current workflow status."""
        entity = QMSAuditPlan.objects.get(id=plan_id)
    ...
    def advance_workflow_stage(self, plan_id: str, action: str, actor_id: str, comment: str = '') -> dict:
        ...
        if not entity.workflow_plan_id:
            raise ValueError("No active workflow for this QMSAuditPlan")
    ...
    @transaction.atomic
    def cancel_workflow_plan(self, plan_id: str, actor_id: str, reason: str = '') -> QMSAuditPlan:
