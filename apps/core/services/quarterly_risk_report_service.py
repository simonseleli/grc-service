"""
Service for Quarterly Risk Management Performance Report workflow integration (FIMS pattern).

Template: grc.quarterly_risk_report_approval
  (5-stage: rmqam_prepare → lsm_submit → management_discussion → committee_review → commission_submit)
"""
import logging
from django.db import transaction
from apps.core.models import QuarterlyPerformanceReport
from apps.infrastructure.external.orchestration_client import OrchestrationClient

logger = logging.getLogger(__name__)


class QuarterlyRiskReportService:

    WORKFLOW_TEMPLATE_CODE = "grc.quarterly_risk_report_approval"

    def __init__(self):
        self.workflow_client = OrchestrationClient()

    @transaction.atomic
    def submit_for_approval(self, report_id: str, submitter_id: str):
        """Submit a Quarterly Performance Report to Work Orchestration Service."""
        entity = QuarterlyPerformanceReport.objects.select_for_update().get(id=report_id)

        if entity.workflow_plan_id:
            logger.info(
                "QuarterlyPerformanceReport %s already has workflow plan %s — skipping",
        ...
        if result and result.plan_id:
            ...
            logger.info(
                "Started workflow plan %s for QuarterlyPerformanceReport %s (stage: %s)",
                result.plan_id, entity.id, result.current_stage_name,
            )
        else:
            logger.error("Failed to start workflow for QuarterlyPerformanceReport %s", entity.id)

        return entity

    def get_workflow_status(self, report_id: str):
        """Get current workflow status."""
        entity = QuarterlyPerformanceReport.objects.get(id=report_id)
    ...
    def advance_workflow_stage(self, report_id: str, action: str, actor_id: str, comment: str = '') -> dict:
        ...
        if not entity.workflow_plan_id:
            raise ValueError("No active workflow for this QuarterlyPerformanceReport")
    ...
    @transaction.atomic
    def cancel_workflow_plan(self, report_id: str, actor_id: str, reason: str = '') -> QuarterlyPerformanceReport:
