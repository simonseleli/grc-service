"""
Service for SettlementDefendant / SettlementPlaintiff workflow integration (FIMS pattern).
Template: grc.legal_settlement_approval
  Stages: settlement_officer_review → legal_manager_settlement_approval
Handles both defendant and plaintiff sides via `side` parameter.
"""
import logging
from django.db import transaction
from apps.core.models import SettlementDefendant, SettlementPlaintiff
from apps.infrastructure.external.orchestration_client import OrchestrationClient
from shared.constants.event_types import LEGAL_SETTLEMENT_EVENTS

logger = logging.getLogger(__name__)


def _publish_settlement_event(event_type, entity, side, actor_id):
    """Best-effort Kafka domain event for settlement lifecycle."""
    try:
        ...
        messaging_service.publish_legal_settlement_event(
            event_type=event_type,
            settlement_id=entity.id,
            additional_data={
                'case_id': str(getattr(entity, case_field, '') or ''),
                'settlement_amount': str(getattr(entity, 'settlement_amount', '') or ''),
                'approved_by': actor_id,
                'user_id': actor_id,
            },
        )
    except Exception as exc:
        logger.warning("Failed to publish %s event for settlement %s: %s", event_type, entity.id, exc)

MODEL_MAP = {
    'defendant': SettlementDefendant,
    'plaintiff': SettlementPlaintiff,
}
...
class LegalSettlementService:
    ...
    def advance_workflow_stage(
        ...
        if result.plan_status == 'completed':
            _publish_settlement_event(LEGAL_SETTLEMENT_EVENTS['SETTLEMENT_APPROVED'], entity, side, actor_id)
